...
**Дополнительно про Элвис_оператор**: Так как `throw` и `return` тоже являются выражениями в **Kotlin**, их также можно использовать справа от _Элвис-оператора_. Это может быть крайне полезным для проверки аргументов функции.
```kt
