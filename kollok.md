...
Помогает перейти наиболее общей и абстрактной концептуальной модели исходной системы к логической, а затем и к физической модели соответствующей программной системы.
...
Описывает функциональность системы с точки зрения внешних акторов. Описывает истории из жизни системы: всевозможные сценарии взаимодействия акторов с системой.

#### Диаграмма классов (Class Diagram)

Используется для визуализации структуры классов, их атрибутов, методов и отношений между классами.

#### Диаграмма последовательности (Sequence Diagram)

Используется для отображения взаимодействия нескольких объектов во времени, обозначая порядок вызовов методов. Используется для моделирования динамического поведения системы.

#### Диаграмма активности (Activity Diagram)

Моделирование потоков работы и деятельности в системе. Визуализирует процесса выполнения операций.

#### Диаграмма состояний (Statechart Diagram)

Моделирует различные состояния объекта или системы и переходы между этими состояниями. Используется для описания поведения объекта или системы в ответ на различные события.

### Какой функционал необходимо описать при моделировании системы заказов доставки посылок? 

Для реализации системы заказов доставки посылок необходим следующий функционал:

**Регистрация и аутентификация** - Клиент может зарегистрироваться в системе и войти в свой аккаунт для управления заказами.

**Заказ посылки** - Клиент может создать заказ, указав адрес отправителя, адрес получателя и описание посылки.

**Отслеживание статуса заказа** - Клиент может отслеживать статус своего заказа (например, "в обработке", "в пути", "доставлен").

**Подтверждение доставки** - После доставки посылки система подтверждает успешное выполнение заказа.

**История заказов** - Клиент может просматривать историю своих предыдущих заказов.

**Уведомления** - Система отправляет уведомления клиенту о статусе заказа (например, по электронной почте или SMS).

### Кратко опишите, какие классы необходимы для моделирования системы заказов посылок? Какие связи необходимы между классами? 

**Класс "Customer" (Клиент)** - Отвечает за информацию о клиенте, его учетные данные, историю заказов.

**Класс "Order" (Заказ)** - Представляет информацию о заказе, такую как адрес отправителя и получателя, описание посылки, статус заказа.

**Класс "Delivery" (Доставка)** - Отвечает за информацию о доставке, такую как статус (в пути, доставлено), подробности о доставке.

**Класс "Notification" (Уведомление)** - Используется для уведомлений клиента о статусе заказа.

#### Примерные связи между классами

**Ассоциация "Customer" -> "Order"**

Один клиент может иметь множество заказов, но каждый заказ принадлежит только одному клиенту.

**Композиция "Order" <>- "Delivery"**

Доставка является частью заказа. Доставка не может существовать без заказов. Причём доставка может включать в себя несколько заказов.

**Композиция "Order" <>- "Notification"**
Уведомление считается частью заказа и не имеет смысла без него.Причём один заказ может иметь множество уведомлений.

### Кратко опишите диаграмму последовательностей для прецедента авторизации пользователя. 

В диаграмме есть линии жизни у двух классов:
- Пользователь (`User`)
- Система Аутентификации (`System`)

**Шаг 1: Запрос на вход ---->**

Сначала от `User` поступает запрос к `System` с данными для авторизации (email и паролем).

**Шаг 2: Ответ системы - - >**

После чего система проверяет данные и возвращает успех, осуществляя процесс аутентификации, когда нужный пользователь нашёлся в базе, иначе возвращает ошибку входа и просит ввести данные для входа ещё раз. Причём разные ответы системы будут являться разными пунктирными стрелками на диграмме.

### Кратко опишите диаграмму активностей для прецедента авторизации пользователя. 

В диаграмме два объекта:
- Пользователь (`User`)
- Система Аутентификации (`System`)

**Начало (Initial Node)**

Первый шаг диаграммы, отражающий начало процесса.

**Шаг 1 - Activity Node**

Пользователь начинает с ввода своего логина и пароля в интерфейс. В зависимости от результатов проверки, система принимает решение о предоставлении доступа или отказе в авторизации. В случае отказа программа попросит ввести данные для входа ещё раз. 

**Конец (Final Node)**

Конец выполнения сценария авторизации пользователя
...
Программа разделяется на процедуры (подпрограммы и функции), которые выполняют определенные действия. Управление программой осуществляется последовательным вызовом процедур.
...
**Достоинства**:
- ООП помогает разделить код на логические блоки, которые способствуют стрктурированию кода и увеличению степени его читабельности.
- Правильно написанным классам можно легко модифицировать интерфейс, добавляя в программу новые и нужные возможности.
- ООП обеспечивает многозадачность различных данных с использованием объектов и классов.

**Недостатки**:
- Для полного понимания ООП требуется много времени на обучение. 
- Проектирование правильной и корректно работающей иерархии классов может быть сложной задачей при наличии небольшого количества опыта и практики.
- Иногда ООП может привести к избыточности кода из-за большого количества классов и объектов, что может затруднить понимание программы.
