...
- После корректного ввода фильма, в консоли появляется список всех доступных на данный фильм сеансов, после чего необходимо ввести `id` сеанса, который пользователь хочет посетить.
...
### Мини-меню фильмов содержит следующие действия
```
1. Посмотреть фильмы в прокате
2. Добавить фильм
3. Удалить фильм
4. Изменить называние фильма
5. Изменить режиссёра фильма
...
При вводе значения `00`, Пользователь выйдет в главное меню.

### 1. Посмотреть фильмы в прокате

При вводе значения `1`, на экране появляются все фильмы, находящиеся в данный момент в прокате. 

### 2. Добавить фильм

- При вводе значения `2`, на экране появляются все фильмы, находящиеся в данный момент в прокате для избежания добавления дубликата. После чего предлагается ввести название нового фильма.
- При корректном вводе названия фильма необходимо ввести режиссёра фильма.
- После успешного ввода фильм добавляется в прокат кинотеарта, а на экран выводится следующее сообщение:
```
Фильм успешно добавлен в прокат! 
```

### 3. Удалить фильм

- При вводе значения `3`, на экране появляются все фильмы, находящиеся в данный момент в прокате для удобства удаления нужного фильма. После чего предлагается ввести название нужного фильма.
- При корректном вводе названия, фильм удаляется из проката кинотеатра, а с ним - все сеансы, показывающие данный фильм, а пользователю выводится следующее сообщение:
```
Фильм успешно удалён из проката!
```

### 4. Изменить название фильма

- При вводе значения `4`, на экране появляются все фильмы, находящиеся в данный момент в прокате для удобства изменения названия фильма. После чего предлагается ввести название нужного фильма.
- При корректном вводе названия, необходимо ввести уже новое название фильма, а при корректном вводе выводится такое сообщение:
```
...
- При вводе значения `4`, на экране появляются все фильмы, находящиеся в данный момент в прокате для удобства изменения названия фильма. После чего предлагается ввести название нужного фильма.
- При корректном вводе названия, необходимо ввести нового режиссёра, а при корректном вводе выводится такое сообщение:
```
Изменения произошли успешно!
```
...
### Мини-меню фильмов содержит следующие действия
```
1. Посмотреть сеансы на фильм
2. Добавить сеанс
3. Удалить сеанс
4. Изменить дату сеанса
5. Изменить временной промежуток сеанса
00. Вернуться в Главное меню
0. Выход
```
...
- При вводе значения `1`, на экране появляются все фильмы, находящиеся в данный момент в прокате. После чего предлагается ввести название фильма.
- После корректного ввода на экране появляются сеансы на нужный фильм.

### 2. Добавить сеанс

- При вводе значения `2`, на экране появляются все фильмы, находящиеся в данный момент в прокате. После чего предлагается ввести название интересущего фильма.
- Далее необходимо ввести дату, время начала и конца сеанса. А при корректном вводе создаётся новый сеанс и пользователь получает следующее сообщение:
```
Сеанс успешно добавлен в базу!
```

### 3. Удалить сеанс

- При вводе значения `3`, на экране появляются все фильмы, находящиеся в данный момент в прокате. После чего предлагается ввести название интересущего фильма.
- Далее необходимо ввести `id` нужного сеанса. При корректном вводе сеанс удаляется и пользователь получает следующее сообщение:
```
Сеанс успешно успешно удалён из базы!
```

### 4. Изменить дату сеанса

- При вводе значения `4`, на экране появляются все фильмы, находящиеся в данный момент в прокате. После чего предлагается ввести название интересущего фильма.
- Далее необходимо ввести `id` нужного сеанса. 
- При корректном вводе сотруднику нужно ввести новую дату сеанса в формате yyyy:mm:dd
- Если вся информация введена корректно, данные о сеансе обновляются, а пользователь получается следующее сообщение:
```
Изменения произошли успешно!
```
...
- При вводе значения `5`, на экране появляются все фильмы, находящиеся в данный момент в прокате. После чего предлагается ввести название интересущего фильма.
- Далее необходимо ввести `id` нужного сеанса. 
- При корректном вводе сотруднику нужно ввести новое время начала и конца сеанса.
- Если вся информация введена корректно, данные о сеансе обновляются, а пользователь получается следующее сообщение:
```
Изменения произошли успешно!
```
